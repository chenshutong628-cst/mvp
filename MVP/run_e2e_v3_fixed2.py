...
def run_render(py_file: Path, class_name: str, quality: str = "l", timeout: int = 900):
    try:
...
def main():
    ...
    # Find most recent successful V3 run with scene designs
    import glob
    v3_runs = sorted(glob.glob(str(RUNS_DIR / "e2e_v3_*")))
    previous_designs_file = None
    
    for run in reversed(v3_runs):
        potential_file = Path(run) / "stage3_designs.json"
        if potential_file.exists():
            previous_designs_file = potential_file
            print(f"Found previous scene designs: {previous_designs_file}")
            break
    
    if previous_designs_file is None:
        print("ERROR: No previous V3 run with scene designs found!")
        print("Please run a full pipeline first to generate scene designs.")
        return 1
